describe('Clinic Note', () => {
  ...
  it('Logs in to staging 63 website and create a clinic note.', () => {
    ...
    cy.get('#login_email').type(Cypress.env('email'))      //input email
    cy.wait(2000)                                          //wait 2 seconds
    cy.get('#login_password').type(Cypress.env('password')) //input password
    cy.wait(2000)                                          //wait 2 seconds
    ...
    //if else statement: if no contanct number then manually type contact number
